...
inline void BundleTriangle_UpdateM6U_LL(Matrix<6> &m6U, Matrix<2, 6> &m26A) {
	for (int r = 0; r < 6; r++)
		for (int c = 0; c <= r; c++)
			m6U(r, c) += m26A.T()(r, 0) * m26A(0, c) + m26A.T()(r, 1) * m26A(1,
					c);
}
inline void BundleTriangle_UpdateM3V_LL(Matrix<3> &m3V, Matrix<2, 3> &m23B) {
	for (int r = 0; r < 3; r++)
		for (int c = 0; c <= r; c++)
			m3V(r, c) += m23B.T()(r, 0) * m23B(0, c) + m23B.T()(r, 1) * m23B(1,
					c);
}
...
int Bundle::Compute(bool *pbAbortSignal) {
	...
	cout << "Final Sigma Squared: " << mdSigmaSquared << " (= " << sqrt(
				mdSigmaSquared) / 4.685 << " pixels.)" << endl;
	return mnAccepted;
}
;
...
bool Bundle::Do_LM_Step(bool *pbAbortSignal) {
	...
	// visodo-AM:
	// std::cerr << "[Bundle]mnCount: " << mnCounter << "\n";
	...
	for (list<Meas>::iterator itr = mMeasList.begin(); itr != mMeasList.end(); itr++) {
		...
		dCurrentError += MEstimator::ObjectiveScore(meas.dErrorSquared,
				mdSigmaSquared);
		/*
		 * visodo-AM: Bugfix of original PTAM:
		 * reset dCurrentError in case it is nan, to avoid infinite loop later
		 * edit 2: not needed any more, as I added a NaN check in InitFromStereo,
		 * but still leave it here commented out
		 */
		//if (isnan(dCurrentError)) dCurrentError = 0.0;
		...
		if (cam.bFixed)
			...
			for (int m = 0; m < 6; m++) {
				...
				v2CamFrameMotion[0] = (v4Motion[0] - v4Cam[0] * v4Motion[2]
						* dOneOverCameraZ) * dOneOverCameraZ;
				v2CamFrameMotion[1] = (v4Motion[1] - v4Cam[1] * v4Motion[2]
						* dOneOverCameraZ) * dOneOverCameraZ;
		...
		for (int m = 0; m < 3; m++) {
			...
			Vector<2> v2CamFrameMotion;
			v2CamFrameMotion[0] = (v3Motion[0] - v4Cam[0] * v3Motion[2]
					* dOneOverCameraZ) * dOneOverCameraZ;
			v2CamFrameMotion[1] = (v3Motion[1] - v4Cam[1] * v3Motion[2]
					* dOneOverCameraZ) * dOneOverCameraZ;
	...
	while (dNewError > dCurrentError && !mbConverged && !mbHitMaxIterations
		...
		for (vector<Point>::iterator itr = mvPoints.begin(); itr
				!= mvPoints.end(); itr++) {
			Point &point = *itr;
		...
		for (unsigned int j = 0; j < mvCameras.size(); j++) {
			...
			for (unsigned int i = 0; i < mvPoints.size(); i++) {
				...
				v6 -= pMeas->m63W * (mvPoints[i].m3VStarInv
						* mvPoints[i].v3EpsilonB);
			}
		...
		for (unsigned int i = 0; i < mvPoints.size(); i++) {
			...
			for (vector<OffDiagScriptEntry>::iterator it =
					p.vOffDiagonalScript.begin(); it
					!= p.vOffDiagonalScript.end(); it++) {
		...
		for (unsigned int i = 0; i < mvPoints.size(); i++) {
			...
			for (unsigned int j = 0; j < mvCameras.size(); j++) {
				...
				if (pMeas == NULL || pMeas->bBad)
					continue;
				v3Sum += pMeas->m63W.T() * vCamerasUpdate.slice(cam.nStartRow,
						6);
			}
		...
		for (unsigned int j = 0; j < mvCameras.size(); j++) {
			if (mvCameras[j].bFixed)
				...
				mvCameras[j].se3CfWNew = SE3<>::exp(vCamerasUpdate.slice(
						mvCameras[j].nStartRow, 6)) * mvCameras[j].se3CfW;
		}
		for (unsigned int i = 0; i < mvPoints.size(); i++)
			mvPoints[i].v3PosNew = mvPoints[i].v3Pos + vMapUpdates.slice(i * 3,
					3);
...
void Bundle::GenerateOffDiagScripts() {
	for (unsigned int i = 0; i < mvPoints.size(); i++) {
		...
		for (set<int>::iterator it_j = p.sCameras.begin(); it_j
				!= p.sCameras.end(); it_j++) {
			int j = *it_j;
			if (mvCameras[j].bFixed)
				continue;
