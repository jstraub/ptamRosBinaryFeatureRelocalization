...
//    DrawCamera(mMap.vpKeyFrames[i]->se3CfromW, true);
  // visodo-AM: draw KeyFramesForDrawing instead of KeyFrames, in order to see old keyframes even if the map has been deleted
  for(size_t i=0; i<mMap.vpKeyFramesForDrawing.size(); i++)
    DrawCamera(mMap.vpKeyFramesForDrawing[i]->se3CfromW, true);
