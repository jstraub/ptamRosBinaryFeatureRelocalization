...
using namespace CVD;
using namespace std;
using namespace GVars3; // to the other classes..
...
Tracker::Tracker(ImageRef irVideoSize, const ATANCamera &c, Map &m,
    MapMaker &mm, const TrackerConfig& config) :
    mMap(m), mMapMaker(mm), mCamera(c),
    mirSize(irVideoSize), mConfig(config), mRand(RND_SEED)
{
  ...
  std::string pathToKeyFrameIDs(mConfig.pathToData);
  std::string pathToFramePoses(mConfig.pathToData);
  std::string pathToRelocalizationFrameNr(mConfig.pathToData);
  std::string pathToFrame2Seq(mConfig.pathToData);
  std::string pathToBriefDescriptors(mConfig.pathToData);
  std::string pathToFeatureStats(mConfig.pathToData);

  mfKeyFrameIDs.open(pathToKeyFrameIDs.append("keyFrameIDs.txt").c_str());
  mfFramePoses.open(pathToFramePoses.append("framePoses.txt").c_str());
  mfRelocalizationFrameNr.open(pathToRelocalizationFrameNr.append("relocalizationFrameNr.txt").c_str());
  mfFrame2Seq.open(pathToFrame2Seq.append("frame2Seq.txt").c_str());
  mfBriefDescriptors.open(pathToBriefDescriptors.append("briefDescriptors.txt").c_str());
  mfFeatureStats.open(pathToFeatureStats.append("featureStats.log").c_str());

  if(!mfKeyFrameIDs.is_open()||
     !mfFramePoses.is_open()||
     !mfRelocalizationFrameNr.is_open()||
     !mfFrame2Seq.is_open()||
     !mfBriefDescriptors.is_open())
  {
    cerr<<"Couldnt open one of the logfiles!"<<endl;
    exit(1);
  }
  cout << "Opened files for writing" << endl;

  mfRelocalizationFrameNr <<"Seq#" // sequence number -> # of frames so far
      <<"\t"<<"overallDt" // measured overall dt for recovery
      <<"\t"<<"forcedWait" // forced waiting time
      <<"\t"<<"tExtract" // Extraction
      <<"\t"<<"tPrep" // preparation LSH
      <<"\t"<<"tPair" // pairing LSH
      <<"\t"<<"tRANSAC" // RANSAC
      <<"\t"<<"tMEst" // MEst
      <<"\t"<<"#queryFeat" // number of query features
      <<"\t"<<"#mapFeat" // number of feature descriptors for matching
      <<"\t"<<"#KeyFrames" // number of Keyframes so far
      <<"\t"<<"#3Dpoints" // number of 3D points
      <<endl;
  mfFeatureStats<<"Seq#"<<"\t"<<"tFAST"<<"\t"<<"tDescExtract"<<"\t"<<"#Feats"<<endl;

  // config for lsh and RANSAC
  uint32_t lshM=14, l=11;
  uint32_t minInliers=10, maxIterations=100;
  double consesusThr=10.0;

  mRelocalisers.push_back(new PTAMRelocaliser(mMap,mCamera));
//  mRelocalisers.push_back(
//      new BriefMEstRelocaliser(mMap,mCamera,LshParams(lshM,l)));
  mRelocalisers.push_back(new PtamBriefRansacRelocaliser(mMap,
      new FourPoint<ATANCamera,BriefData>(mCamera,consesusThr),
      LshParams(l,lshM),
      RansacParams(minInliers,maxIterations),mConfig.featureType));
...
Tracker::~Tracker()
{
  mfKeyFrameIDs.close();
  mfFramePoses.close();
  mfRelocalizationFrameNr.close();
  mfFrame2Seq.close();
  mfFeatureStats.close();
}
...
void
Tracker::TrackFrame(Image<byte> &imFrame, vector<double> &vOrientation,
    vector<double> &vVelocity, vector<double> &vAcceleration, int32_t &t_sec,
    int32_t &t_nsec, int32_t &seq, bool& initialized, bool& simTrackingLoss, bool bDraw)
{
  static bool saveSomeBRIEFs=false;

  simTrackingLoss=false; // standard -> do not sleep!
  ...
  // visodo-AM: output the imu data for debugging
  if (vAcceleration.size() == 3)
  {
    std::cout << "---" << endl;
//	  std::cout << "sec: " << t_sec << " nsec: " << t_nsec << "\npose:\n" << mse3CamFromWorld;
//	  std::cout << "Ori_X:" << vOrientation[0] << " Ori_Y:" << vOrientation[1] << " Ori_Z:" << vOrientation[2] << " Ori_W:" << vOrientation[3] << endl;
//	  std::cout << "Vel_X:" << vVelocity[0] << " Vel_Y:" << vVelocity[1] << " Vel_Z:" << vVelocity[2] << endl;
//	  std::cout << "Acc_X:" << vAcceleration[0] << " Acc_Y:" << vAcceleration[1] << " Acc_Z:" << vAcceleration[2] << endl;
    std::cout << t_sec << " " << t_nsec << endl;
    std::cout << mse3CamFromWorld;
    std::cout << vOrientation[0] << " " << vOrientation[1] << " "
        << vOrientation[2] << " " << vOrientation[3] << endl;
    cout << vVelocity[0] << " " << vVelocity[1] << " " << vVelocity[2]
        << endl;
    cout << vAcceleration[0] << " " << vAcceleration[1] << " "
        << vAcceleration[2] << endl;
  }
  ...
  tExtract.toc();
  mCurrentKF.mSeq=mSeq; // set the sequence number

//  {// save statistics about the number of extracted features per frame pyramid level
//    mfFeatureStats<<mSeq<<"\t";
//    for(uint32_t i=0; i<LEVELS; ++i)
//      mfFeatureStats<<mCurrentKF.aLevels[i].vCandidates.size()<<"\t";
//    mfFeatureStats<<endl;
//  }
  ...
  {// Save Image
    /*fileName << mConfig.pathToData << "frame_";
    fileName.fill('0'); fileName.width(5);
    fileName << right << mnFrame << ".png";
    cout << fileName.str() << endl;
    CVD::img_save(mCurrentKF.aLevels[0].im, fileName.str());*/
    mfFrame2Seq<<mnFrame<<"\t"<<mSeq<<endl;
  }
  ...
  if (mMap.IsGood())
  {
    if(mConfig.manualRelocalize && ((mnFrame % mConfig.framesBetweenManualReloc) == 0))
    {
      // manual relocalization -> sleep now and then exit without doing anything
      // since we need to get an frame from after the 10s sleep to do the relocalisation with!
      cout<<"Manual relocalization -> sleep(10) "<<endl;
      mnLostFrames=100; // set to value higher than 3 so that next time this function is executed relocalisation is performed
      {// -------- Save Camera Pose
        mfFramePoses << mSeq << "\t" //      mfFramePoses << mnFrame << "\t"
            << int64_t(t_sec) * 1000000000 + int64_t(t_nsec) << "\t"
            << serializeForOutput(mse3CamFromWorld.inverse())<<endl;//mCurrentKF.se3CfromW)<<endl;
        mfFramePoses.flush();
      }
      simTrackingLoss=true;
//      sleep(10);
      return; // leave here so we get a frame from after the sleep to do localisation with that one
    }

    if (mnLostFrames < 6) // .. but only if we're not lost!
    {
      ...
      // save BriefDescriptors on the way for Retrieval tests
      if((mnFrame-mnLastKeyFrameDropped)%10==9 && saveSomeBRIEFs)
      {
        mfFeatureStats << mSeq << "\t" << tExtract.lastDt()<< "\t";
        BinaryExtractor be(mConfig.featureType);
        be.saveDescriptors(mfBriefDescriptors,mfFeatureStats,mCurrentKF,mSeq);
        saveSomeBRIEFs=false;
        mfFeatureStats <<endl;
      }

      if(false)
      {// -------- Save KeyPoint Positions
        fileName.str(""); fileName << mConfig.pathToData << "keyPointMeasurements_";
        fileName.fill('0'); fileName.width(5);
        fileName << right << mnFrame << ".txt";
        cout << fileName.str() << endl;
        VectorToFile v2f(fileName.str().c_str());
        v2f.writeBatch(mCurrentKF.mMeasurements,VectorToFile::second);
      } // need those brackets because they will call the destructor for local variables (this closes the output file)

    } else // what if there is a map, but tracking has been lost?
    {
      // visodo-AM: initial change: no recovery, but immediately deletion of map and re-initialization
      // now: re-integrate recovery, but tune it, so that it not always returns true
      mMessageForUser << "** Attempting recovery **.";
      float dt=0.0;
      if (AttemptRecovery(dt))
      {
        cout<<" Sleeping "<<mConfig.dtAfterRelocDone<<"ms"<<endl;
        usleep(ceil(mConfig.dtAfterRelocDone*1000.0));
        mfRelocalizationFrameNr <<mSeq // sequence number -> # of frames so far
            <<"\t"<<dt // measured overall dt for recovery
            <<"\t"<<mConfig.dtAfterRelocDone // forced waiting time
            <<"\t"<<mRelocalisers[mConfig.relocType]->getRelocResults().tExtract // Extraction
            <<"\t"<<mRelocalisers[mConfig.relocType]->getRelocResults().tPrep // preparation LSH
            <<"\t"<<mRelocalisers[mConfig.relocType]->getRelocResults().tPair // pairing LSH
            <<"\t"<<mRelocalisers[mConfig.relocType]->getRelocResults().tRANSAC // RANSAC
            <<"\t"<<mRelocalisers[mConfig.relocType]->getRelocResults().tMEst // MEst
            <<"\t"<<mRelocalisers[mConfig.relocType]->getRelocResults().querySize // number of query features
            <<"\t"<<mRelocalisers[mConfig.relocType]->getRelocResults().mapSize // number of feature descriptors for matching
            <<"\t"<<mMap.vpKeyFrames.size() // number of Keyframes so far
            <<"\t"<<mMap.vpPoints.size() // number of 3D points
            <<endl;
        mMessageForUser << "** Attempting recovery TRUE **.";
        TrackMap(); // TODO: can I simply take that out in case I am using BRIEF relocalize?
...
//	RenderGrid();
  ...
  // visodo-AM: automatic tracking instead of spacebar pressing; and only after the video has run 10 frames
  if (mnFrame > 10)
  {
    TrackForInitialMapAuto();
    // visodo-AM: debug output
    std::cout << "we're lost\n";
  }
...
bool Tracker::AttemptRecovery(float& dt)
{
  bool bRelocGood = false;
  dt=0.0;
  if(mConfig.relocType<RelocType::NUMBER_RELOC_TYPES)
  {
#ifndef TIME_MEASUREMENT
    cout<<"Relocalizing using relocType="<<mConfig.relocType<<endl;
#endif
    bRelocGood = mRelocalisers[mConfig.relocType]->AttemptRecovery(mCurrentKF,dt);
  }else{
    return false; // this relocalizer does not exist.
  }
  if (bRelocGood)
  {
    SE3<> se3Best = mRelocalisers[mConfig.relocType]->BestPose();
...
void
Tracker::GUICommandCallBack(void* ptr, string sCommand, string sParams)
{
  Command c;
  c.sCommand = sCommand;
  c.sParams = sParams;
...
void
Tracker::GUICommandHandler(string sCommand, string sParams) // Called by the callback func..
{
  if (sCommand == "Reset")
  {
    Reset();
...
// visodo-AM: Modification of TrackForInitialMap with automatic initialization
void
Tracker::TrackForInitialMapAuto()
{
  ...
  // if tracking has been executed previously, reset (once). In the next loop,
  // we will fall into the trailtracking_advance case
  if (mnInitialStage == TRAIL_TRACKING_COMPLETE
      && mNumFramesForInitialTracking > 0) mnInitialStage =
  ...
  if (mnInitialStage == TRAIL_TRACKING_NOT_STARTED)
  {
    ...
    mNumFramesForInitialTracking = 0;
  };

  if (mnInitialStage == TRAIL_TRACKING_STARTED)
  {
    int nGoodTrails = TrailTracking_Advance(); // This call actually tracks the trails
    mNumFramesForInitialTracking++;
    mMessageForUser << "numFrames: " << mNumFramesForInitialTracking << endl;

    if (mNumFramesForInitialTracking > 25) // move at least 10 frames forward

    {
      mMessageForUser << "Starting Initialization" << endl;
      if (nGoodTrails > 10)
      {
        ...
        // Using the mMap.vpKeyFrames vector since the MapMaker makes internal copies of the
        // KeyFrames! I need those pointers for BriefData.
        // after InitFromStereo there will be two KeyFrames in the vector mMap.vpKeyFrames
        assert(mMap.vpKeyFrames[0]!=NULL);
        assert(mMap.vpKeyFrames[1]!=NULL);
        BinaryExtractor be(mConfig.featureType);
        be.extractForMapPoints(*(mMap.vpKeyFrames[0]),mFirstKFSeq);//(&mFirstKF);
        be.extractForMapPoints(*(mMap.vpKeyFrames[1]),mSeq);//(&mCurrentKF);
        mfKeyFrameIDs<<mFirstKFSeq<<endl<<mSeq<<endl;

      } else
      {
        Reset();
        mMessageForUser << "Not enough trails found" << endl;
        mNumFramesForInitialTracking = 0;
        mnInitialStage = TRAIL_TRACKING_NOT_STARTED;
      }
    }
...
Tracker::TrailTracking_Start()
{
  ...
  { // so that we can choose the image corners with max ST score
    ...
    if (!mCurrentKF.aLevels[0].im.in_image_with_border(c.irLevelPos,
        MiniPatch::mnHalfPatchSize)) continue;
  ...
  for (unsigned int i = 0; i < vCornersAndSTScores.size() && nToAdd > 0; i++)
  {
    if (!mCurrentKF.aLevels[0].im.in_image_with_border(
        vCornersAndSTScores[i].second, MiniPatch::mnHalfPatchSize)) continue;
    Trail t;
...
Tracker::TrailTracking_Advance()
{
  ...
  for (list<Trail>::iterator i = mlTrails.begin(); i != mlTrails.end();)
  {
    ...
    if (bFound)
    {
      ...
      if ((irBackWardsFound - irStart).mag_squared() > 2) bFound = false;
...
Tracker::TrackMap()
{
  ...
  for (unsigned int i = 0; i < mMap.vpPoints.size(); i++)
  {
    ...
    if (TData.nSearchLevel == -1) continue; // a negative search pyramid level indicates an inappropriate warp for this view, so skip.
  ...
  for (int i = 0; i < LEVELS; i++)
    random_shuffle(avPVS[i].begin(), avPVS[i].end(),mRand);
...
int
Tracker::SearchForPoints(vector<TrackerData*> &vTD, int nRange, int nSubPixIts)
{
  int nFound = 0;
...
Vector<6>
Tracker::CalcPoseUpdate(vector<TrackerData*> vTD, double dOverrideSigma,
    bool bMarkOutliers)
{
  ...
  for (unsigned int f = 0; f < vTD.size(); f++)
  {
    ...
    TD.v2Error_CovScaled = TD.dSqrtInvNoise * (TD.v2Found - TD.v2Image); // found position - projected position
    vdErrorSquared.push_back(TD.v2Error_CovScaled * TD.v2Error_CovScaled);
  };

  // No valid measurements? Return null update.
  if (vdErrorSquared.size() == 0) return makeVector(0, 0, 0, 0, 0, 0);
  ...
  for (unsigned int f = 0; f < vTD.size(); f++)
  {
    ...
    if (dWeight == 0.0)
    {
      if (bMarkOutliers) TD.Point.nMEstimatorOutlierCount++;
      continue;
    } else if (bMarkOutliers) TD.Point.nMEstimatorInlierCount++;
...
{
  mfKeyFrameIDs << mCurrentKF.mSeq<<endl; //mnFrame << "\t"; // output to file

  // original code of AddKeyFrame
  KeyFrame* pMapInternalKF=mMapMaker.AddKeyFrame(mCurrentKF);
  mnLastKeyFrameDropped = mnFrame;

  // extract Brief Features - needs Map internal KeyFrame pointer.
  BinaryExtractor be(mConfig.featureType);
  be.extractForMapPoints(*pMapInternalKF,mSeq);

  if(false)// we do this at the very end for all KeyFrames (after full optimization of 3D positions
  {// print briefFeature 3D and image positions alongside with the descriptor
    char buf[60]; sprintf(buf,"../../data/kFrameBriefDescriptors%05d.txt",mSeq);
    std::ofstream briefOut(buf);
    std::map<MapPoint*, Measurement>& meas=pMapInternalKF->mMeasurements;
    for(std::map<MapPoint*, Measurement>::iterator it=meas.begin();it!=meas.end();it++)
    {
      briefOut<<it->first->pBData->printForMatlab()<<endl;
    }
    briefOut.close();
  }
}
