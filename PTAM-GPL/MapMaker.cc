...
MapMaker::MapMaker(Map& m, const ATANCamera &cam)
  : mMap(m), mCamera(cam), mRnd(RND_SEED)
{
...
void MapMaker::run()
{
  ...
  while(!shouldStop())  // ShouldStop is a CVD::Thread func which return true if the thread is told to exit.
	...
	continue;
      ...
      if(mbBundleConverged_Recent && QueueSize() == 0) {
    	  //TODO: what does this mean? commented out because its cluttering everything
          // cerr << "ReFindNewlyMade" << endl;
    	  ReFindNewlyMade();
      }
      ...
      if(mbBundleConverged_Recent && !mbBundleConverged_Full && QueueSize() == 0)
        BundleAdjustAll();
    	  // visodo-AM: due to perfomance reasons not all keyframes
    	//BundleAdjustRecent();
			      ...
			      SE3<> &se3TrackerPose)
{
  ...
  SE3<> se3;
  bool bGood;
  HomographyInit HomographyInit(mRnd);
  ...
  *pkSecond = kS;
  ...
  mMap.vpKeyFramesForDrawing.push_back(pkFirst); // visodo-AM: also add the KFForDrawing
  mMap.vpKeyFramesForDrawing.push_back(pkSecond);
  pkFirst->MakeKeyFrame_Rest();
  pkSecond->MakeKeyFrame_Rest();
  
  // visodo-AM: check for points with NaN v3WorldPoses, then break (to avoid crash in Bundle Adjustment)
  for(unsigned int i=0; i<mMap.vpPoints.size(); i++) {
	  if (mMap.vpPoints[i]->v3WorldPos != mMap.vpPoints[i]->v3WorldPos) {
		  return false;
	  }
  }
  // end of visodo-AM
  ...
  int numIterations = 0; // visodo-AM: add counter for bundle adjustment iterations and do maximal 5 iterations
  while(!mbBundleConverged_Full)
	...
	return false;
      if (numIterations++ > 5) {
    	  mbBundleConverged_Full = true;
    	  std::cerr << "[InitFromStereo] manual break after 5 iterations\n";
    	  break;
      }
    }
  ...
  // visodo-AM: set last known pose after reinitialization
  ApplyGlobalTransformationToMap(se3TrackerPose.inverse()); // TODO: problem if trackerpose is nan
  mMap.bGood = true;
...
void MapMaker::ApplyGlobalTransformationToMap(SE3<> se3NewFromOld)
{
  for(unsigned int i=0; i<mMap.vpKeyFrames.size(); i++)
  {
    ...
    mMap.vpKeyFramesForDrawing[i]->se3CfromW = mMap.vpKeyFramesForDrawing[i]->se3CfromW * se3NewFromOld.inverse(); // visodo-AM: do the same for our KeyFramesForDrawing
  }
...
void MapMaker::AddKeyFrameFromTopOfQueue()
{
  ...
  mMap.vpKeyFramesForDrawing.push_back(pK); // visodo-AM: a copy for drawing which is not deleted
